	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift(dx, dy, e) =>
			(case eval_prog(e, env) of
                ...
                | _ => raise Impossible("Shift not applicable for this type"))
...
fun preprocess_prog e =
    case e of
        LineSegment (x1, y1, x2, y2) =>
            if x1 >= x2 andalso not
               (real_close(x1, x2) andalso (y1 < y2 + epsilon)) then
                preprocess_prog(LineSegment(x2, y2, x1, y1))
            else if real_close_point (x1, y1) (x2, y2) then
                Point(x1, y1)
            else
                e
       | Intersect(e1, e2) =>
